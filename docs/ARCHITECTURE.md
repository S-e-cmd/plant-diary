...
- `client/rotation-utils.js` — staged rotation-plan detection, active rotation filtering, and rotation actual-history ordering helpers; not yet wired into runtime.
...
6. Log date, log filter/page, quick-input, and rotation pure helpers still run from current inline functions in `index.html`; staged modules are not production runtime dependencies yet.
...
- Rotation extraction preserves `type === plan`, non-empty rotation name, positive numeric rotation order, completed/cancelled exclusion, and ascending date ordering of matching actuals.
...
The staged log-date switch uses `scripts/log-date-extraction-transform.mjs` as an exact-match fail-closed transform. Quick-input, log-filter, and rotation modules currently have unit/semantic-parity tests only and must not be manually wired by broad replacement.
