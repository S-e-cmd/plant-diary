...
- `api-client.js` — same-origin `/api` へのPOST、JSON検証、APIエラー変換を共通化するブラウザ側通信helper。外部UIモジュールは独自fetchを複製せずこれを使う。
...
- `log-tools-ui.js` — ログ画面の「履歴分析」「資材・薬剤の使用履歴」「削除済みの記録」を既存GAS契約へ接続するUI責務。通信は `api-client.js` を使用し、分析は `getAnalysis`、削除済み一覧は明示的なfull `bootstrap`、復元は `restore` を使う。軽量起動データを削除済み一覧の正本として扱わない。
...
- API client: `scripts/test-api-client.mjs`
...
データ処理runtimeはGAS業務ロジックや無関係なDOM責務を取り込みません。外部UIモジュールのsame-origin API通信は `api-client.js` に集約し、ログ画面固有のUI操作は `log-tools-ui.js` に分離します。DOM契約、LocalStorageキー、API payload、20件ページング、ローテーション履歴、既存表示文言を維持します。
